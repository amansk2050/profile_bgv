...
then
    ...
    aws lambda publish-version --function-name $projname$1

	# aws s3 --region eu-central-1 cp $1.zip s3://facilitybook-backend/deploy/src/$1.zip
	# name="arn:aws:lambda:eu-central-1:188097494660:function:$projname$1"
	# aws lambda update-function-code --function-name $name --s3-bucket facilitybook-backend --s3-key deploy/src/$1.zip --region eu-central-1
